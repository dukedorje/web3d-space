...
`/mesh` may load a world-model document (HDRI, imported GLB, optional
splat-cache refs, LOD distances) beside coverage paint. Capture enters
as GLB or splat cache, not as an in-browser trainer. A missing asset
fails visibly. Coverage still works without a world document
(`?noworld`).
