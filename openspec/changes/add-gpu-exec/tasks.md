...
- [x] Extract a frame/compute kernel from `animation-loop.ts` that
      does not import `boid-buffers`, `boid-compute`, or `boid-render`
- [x] Keep `/boids` running on that kernel (plugin wiring)
- [x] Unit tests: kernel start/stop/dispatch without boid-steering;
